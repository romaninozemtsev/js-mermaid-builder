...
`;
    ...
    test('Parser round-trips quick style chart', () => {
        const original = new Chart('test1')
            .addNode('user')
            .addNode('client')
            .addNode('server')
            .addNode('database')
        ...
        const parsed = Chart.parse(original);
        expect(parsed.toString()).toBe(original);
    });

    test('Parser round-trips nested chart with link styles', () => {
        const chart = new Chart('Test', ChartDir.TB);
        const node1 = new ChartNode('this is my node', NodeShape.HEXAGON, 'my-node');
        const node2 = new ChartNode('this is my second node');
        chart.addNode(node1).addNode(node2);
